...
void Util::writeResult(worksheet *ws, xf_t *xf, AnalyzeResult res)
{
    ...
    ws->label(10,1,"满载电费(元)",xf);
    ws->label(10,2,Util::ftos(res.load_charge).toStdString(),xf);
    ws->label(10,5,"空载电费(元)",xf);
    ws->label(10,6,Util::ftos(res.unload_chargd).toStdString(),xf);

    ws->label(11,1,"满载耗电占比(%)",xf);
    ws->label(11,2,Util::ftos(res.load_charge_radio).toStdString(),xf);
    ws->label(11,5,"空载耗电占比(%)",xf);
    ws->label(11,6,Util::ftos(res.unload_charge_radio).toStdString(),xf);
}
